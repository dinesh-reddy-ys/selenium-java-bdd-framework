 ...
 *
 * <p>This class provides helper methods to interact with different types
 * of select controls shown on the page:
 * - a react-style searchable select (custom dropdown)
 * - a standard multi-select (<select multiple>)
 * - a legacy/old-style single select element
 *
 * The methods wait for elements to be clickable before interacting to
 * make tests more stable against timing issues.
 */
public class SelectMenuPage {
    ...
    /**
     * Selects the specific value "Group 1, option 1" from the custom dropdown.
     * This opens the dropdown then clicks the option.
     */
    ...
    /**
     * Selects multiple options from a standard multi-select element.
     * The method assumes the multi-select supports selecting by visible text.
     * It selects "Volvo" and "Saab" as part of the happy-path test.
     */
    ...
    /**
     * Clicks the react-style multi-select placeholder to open its options.
     * Uses Actions to move to the element to avoid potential overlay/visibility issues.
     */
    public void selectMultiSelectDropdownOption() {
        wait.until(ExpectedConditions.elementToBeClickable(multiSelectDropdown));
        Actions actions = new Actions(driver);
        // Move to the element and click — helps when element is off-screen or overlapped
        actions.moveToElement(multiSelectDropdown).click().perform();
    }

    /**
     * Verifies that the react-style multi-select control contains at least one option.
     * This method performs a DOM query for the option nodes and asserts the count > 0.
     *
